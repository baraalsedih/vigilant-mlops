...
ML_SERVE_DIR = _REPO_ROOT / "apps" / "ml-serve"
_BACKEND_DB = _REPO_ROOT / "apps" / "backend" / "core" / "database" / "vigilant.db"
_SEED_SQL = _REPO_ROOT / "apps" / "backend" / "core" / "database" / "seed_data.sql"

STEPS = 6
STAGE_EVALUATE_DATA = "evaluate-data"
STAGE_EVALUATE_MODEL = "evaluate-model"
STAGE_EVALUATE_DRIFT = "evaluate-drift"
STAGE_EXPORT_BASELINES = "export-baselines"
VALID_STAGES = {STAGE_EVALUATE_DATA, STAGE_EVALUATE_MODEL, STAGE_EVALUATE_DRIFT, STAGE_EXPORT_BASELINES}
...
def _export_baselines(step: int) -> None:
    import duckdb as _duckdb

    header = f"[{step}/{STEPS}] export-baselines"
    print(f"\n=== {header} ===")

    if not _BACKEND_DB.exists():
        print(f"  ✗ Database not found: {_BACKEND_DB}", file=sys.stderr)
        sys.exit(1)

    try:
        conn = _duckdb.connect(str(_BACKEND_DB), read_only=True)
        rows = conn.execute(
            "SELECT feature_name, stats_json FROM feature_stats ORDER BY feature_name"
        ).fetchall()
        conn.close()
    except Exception as exc:
        print(f"  ✗ Failed to read feature_stats: {exc}", file=sys.stderr)
        sys.exit(1)

    if not rows:
        print("  — feature_stats is empty, nothing to export")
        return

    lines = ["-- Auto-generated by scripts/seed.py — do not edit manually", ""]
    for feature_name, stats_json in rows:
        safe_name = feature_name.replace("'", "''")
        safe_json = str(stats_json).replace("'", "''")
        lines.append(
            f"INSERT OR REPLACE INTO feature_stats (feature_name, stats_json)"
            f" VALUES ('{safe_name}', '{safe_json}');"
        )
    lines.append("")

    _SEED_SQL.write_text("\n".join(lines), encoding="utf-8")
    rel = _SEED_SQL.relative_to(_REPO_ROOT)
    print(f"  ✓ Exported {len(rows)} rows → {rel}")
    print()
...
def main() -> None:
    ...
    if STAGE_EXPORT_BASELINES in skip:
        _skip(6, "export-baselines")
    else:
        _export_baselines(6)
